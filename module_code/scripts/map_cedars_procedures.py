...
def trace_code_connections(starting_code: str, mapping_df: DataFrame) -> List[str]:
    ...
    while len(codes_to_visit) > 0:
        ...
        if not current_code in visited_codes:
            ...
            neighbours_forwards = mapping_df[mapping_df['PROC_CODE'] == current_code]['OT_PROC_CODE'].to_list()
            neighbours_backwards = mapping_df[mapping_df['OT_CODE_TYPE'] == current_code]['PROC_CODE'].to_list()
...
def create_and_serialize_cpt_mapping_dict(args: Namespace, procedures:DataFrame, mapping_df:DataFrame) -> None:
    cpt = CPT()
    ...
    with open(join(args.cedars_crrt_data_dir, 'Procedures_Code_Mapping.pkl'), 'wb') as f:
...
def main():
    ...
    proc_mapping = read_excel(join(args.cedars_crrt_data_dir, 'karumanchi_00001867_proc_code_bridge_table.xlsx'))
