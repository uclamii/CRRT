...
def align_outcomes(args: Namespace):
    ...
    df["Start Date"] = pd.DatetimeIndex(df["End Date"])-pd.to_timedelta(df['CRRT Total Days']-1, unit='d')
...
def rename_features(args: Namespace):
    ...
    for input_feature, output_feature in mapping.items():
        if output_feature is not None:
            ...
            df.rename(columns={'PAT_ID': "IP_PATIENT_ID", 'PAT_ENC_CSN_ID':'IP_ENCOUNTER_ID'}, inplace=True)
