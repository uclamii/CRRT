...
def jelly_pairwise_diff(left:list, right:list) -> DataFrame:
    ...
    distances = DataFrame({'Distance':results.values()}, index = results.keys()).sort_values('Distance')
    return distances
...
def fuzzy_pairwise_diff(left:list, right:list, mode:str = None) -> DataFrame:
    ...
    distances = DataFrame({'Distance':results.values()}, index = results.keys()).sort_values('Distance', ascending=False)
    return distances

def create_and_serialize_labs_mapping_dict(args: Namespace, cedars_labs: set, ucla_labs: set) -> None:

    matching_cedars_lab_names = []
    matching_ucla_lab_names = []

    ucla_labs['REGEX_COMPONENT_NAME'] = ucla_labs['COMPONENT_NAME'].str.replace('[^a-zA-Z0-9]', ' ', regex=True)
    ucla_labs['REGEX_COMPONENT_NAME'] = ucla_labs['REGEX_COMPONENT_NAME'].apply(lambda x: re.sub(' +', ' ', x).strip())

    cedars_labs['REGEX_COMPONENT_NAME'] = cedars_labs['COMPONENT_NAME'].str.replace('[^a-zA-Z0-9]', ' ', regex=True)
    cedars_labs['REGEX_COMPONENT_NAME'] = cedars_labs['REGEX_COMPONENT_NAME'].apply(lambda x: re.sub(' +', ' ', x).strip())
    
    only_cedars = (set(cedars_labs['REGEX_COMPONENT_NAME'].unique())).difference(set(ucla_labs['REGEX_COMPONENT_NAME'].unique()))
    ...
    for i, row in pairs_with_one_char_diff.iterrows():
        ...
        diff_chars = set(cedars_characters.keys()).symmetric_difference(set(ucla_characters.keys()))
    ...
    # Get overlap
    same = list(set(cedars_labs['REGEX_COMPONENT_NAME']).intersection(ucla_labs['REGEX_COMPONENT_NAME']))
    ...
    cedars_mapping_df = cedars_labs[cedars_labs['REGEX_COMPONENT_NAME'].isin(matching_cedars_lab_names)]
    ucla_mapping_df = ucla_labs[ucla_labs['REGEX_COMPONENT_NAME'].isin(matching_ucla_lab_names)]
    
    # Map cedars to adjusted labs
    cedars_lab_mapping = dict(
        zip(cedars_mapping_df["COMPONENT_NAME"], cedars_mapping_df["REGEX_COMPONENT_NAME"])
    )

    # Map cedars adjusted labs to ucla adjusted labs
    REGEX_lab_mapping = dict(
        zip(matching_cedars_lab_names, matching_ucla_lab_names)
    )
    ...
    for key in cedars_lab_mapping:
        final_mapping[key] = ucla_lab_mapping[REGEX_lab_mapping[cedars_lab_mapping[key]]]
...
def main():
    ...
    cedars_labs = read_files_and_combine([FILE_NAMES["labs"]], args.cedars_crrt_data_dir)
